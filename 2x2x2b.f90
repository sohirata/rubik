...
!    QUARTER-TURN METRIC
     IF (METRIC==1) THEN
      ...
      DO ITURN=1,NTURN
       ...
       IF (ALGORITHM==1) THEN
        IDIST=ISTEP
        CALL BISECTION(JCONFIG,IDIST,ITURN)
        IF (IDIST==-1) THEN
         INEW=INEW+1
         IF (INEW > N) THEN 
          WRITE(6,'(A)') 'ERROR 1'
...
!    HALF-TURN METRIC
   ...
   IF (ILIST/=N) THEN
    ...
    STOP
   ELSE
    WRITE(6,'(A,I0)') 'RADIUS = ',ISTEP
   ENDIF
   TURNSTAT=0
   DO I=1,ILIST
    DO J=1,ISTEP
     ITURN=TURN(I)-(TURN(I)/100)*100
...
SUBROUTINE BISECTION(CONFIG,IDIST,ITURN)
...
!  WRITE(6,*) 'CONFIG ',CONFIG
   ...
   IF (ILIST==1) THEN
    ...
    ELSE
     ...
     LIST(ILIST)=CONFIG
     DIST(ILIST)=JDIST
     TURN(ILIST)=TURN(ILIST)*100+ITURN
    ENDIF
    RETURN
   ENDIF
