     ...
     ELSE IF (METRIC==3) THEN
      ...
      DO ITURN=1,NTURN
       IF (ITURN==1) THEN
        ...
        CALL R(JCUBOID,KCUBOID,.TRUE.)
        CALL L(KCUBOID,ICUBOID,.FALSE.)
        CALL L(ICUBOID,JCUBOID,.FALSE.)
       ENDIF
       IF (ITURN==2) THEN
        CALL D(ICUBOID,JCUBOID,.TRUE.)
        CALL D(JCUBOID,KCUBOID,.TRUE.)
        CALL U(KCUBOID,ICUBOID,.FALSE.)
        CALL U(ICUBOID,JCUBOID,.FALSE.)
       ENDIF
       IF (ITURN==3) THEN
        CALL F(ICUBOID,JCUBOID,.TRUE.)
        CALL F(JCUBOID,KCUBOID,.TRUE.)
        CALL B(KCUBOID,ICUBOID,.FALSE.)
        CALL B(ICUBOID,JCUBOID,.FALSE.)
       ENDIF
       JFOUND=IEXIST(JCUBOID)
       IF (JFOUND==0) THEN
        ILIST=ILIST+1
