...
! SO HIRATA, UNIVERSITY OF ILLINOIS AT URBANA-CHAMPAIGN, JANUARY 2024.
   ...
   INTERFACE
      LOGICAL FUNCTION LEXIST(CONFIG,ANYLIST,ILISTSIZE)
         INTEGER(16) :: CONFIG
         INTEGER(4)  :: ILISTSIZE
         INTEGER(16) :: ANYLIST(*)
      END FUNCTION
...
!  INTEGER(16) :: JJJ
   ...
   INTEGER(4),PARAMETER :: ALGORITHM=1 ! 1 SORTED; 2 UNSORTED
   INTEGER(4),PARAMETER :: N=7*6*5*4*3*2*1*(3**6)
   INTEGER(4)           :: METRIC   ! 1 QUARTER-TURN; 2 HALF-TURN; 3 SEMI-QUARTER-TURN; 4 COMPOUND-TURN
!  INTEGER(4),PARAMETER :: METRIC=4 ! 1 QUARTER-TURN; 2 HALF-TURN; 3 SEMI-QUARTER-TURN; 4 COMPOUND-TURN
   INTEGER(16) :: LIST(1:N),SUBLIST(1:N),NEWLIST(1:N)
   INTEGER(4)  :: ILIST,ISUB,INEW
   INTEGER(16) :: ICONFIG,JCONFIG
   INTEGER(4)  :: ICUBOID(1:21),JCUBOID(1:21),KCUBOID(1:21)
   INTEGER(4)  :: ISTEP,ITURN
   LOGICAL     :: LFOUND
   ...
   ELSE IF (METRIC==4) THEN
    WRITE(6,'(A)') 'COMPOUND-TURN METRIC'
   ELSE
...
!  WRITE(6,'(I0,A,I0)') JCONFIG,' R^PRIME ',ICONFIG
   ...
   ISUB=1
   INEW=0
   WRITE(6,'(3(A,I10))') 'STEP ',ISTEP,' NEW CONFIGS ',INEW,' CUMMULATIVE CONFIGS ',ILIST
...
!    QUARTER-TURN METRIC
     IF (METRIC==1) THEN
      DO ITURN=1,6
       ...
       CALL COMBINE(JCONFIG,JCUBOID)
       IF (ALGORITHM==1) THEN
        CALL BISECTION(JCONFIG,LIST,ILIST,LFOUND)
        IF (.NOT.LFOUND) THEN
         INEW=INEW+1
         IF (INEW > N) THEN 
          WRITE(6,'(A)') 'ERROR 1'
        ...
        ENDIF 
       ELSE IF (ALGORITHM==2) THEN
        IF (.NOT.LEXIST(JCONFIG,LIST,ILIST)) THEN
         ILIST=ILIST+1
         IF (ILIST > N) THEN
          WRITE(6,'(A)') 'ERROR 2'
          STOP
...
! RETURN TRUE IF CONFIG EXISTS IN THE LIST
   ...
   INTEGER(16) :: CONFIG
   INTEGER(4)  :: ILISTSIZE,I
   INTEGER(16) :: ANYLIST(*)

!  ! DEBUG
!  IF (ILISTSIZE < 40) THEN
!   WRITE(6,*) 'CONFIG  =',CONFIG
!   WRITE(6,*) 'LISTSIZE=',ILISTSIZE
!   DO I=1,ILISTSIZE
!    WRITE(6,*) ANYLIST(I),(CONFIG==ANYLIST(I))
!   ENDDO
!  ENDIF

   LEXIST=.FALSE.
   DO I=1,ILISTSIZE
    IF (CONFIG==ANYLIST(I)) THEN
     LEXIST=.TRUE.
     EXIT
    ENDIF
   ENDDO
...
SUBROUTINE BISECTION(CONFIG,ANYLIST,ILISTSIZE,LFOUND)
! SEARCH CONFIG IN THE SORTED LIST AND IF NOT FOUND INSERT CONFIG IN THE LIST

   INTEGER(16) :: CONFIG
   INTEGER(4)  :: ILISTSIZE,I,J
   INTEGER(16) :: ANYLIST(*) ! ASSUMED TO BE SORTED ON INPUT
...
!  WRITE(6,*) 'CONFIG ',CONFIG
   ...
   IF (ILISTSIZE==1) THEN
    IF (CONFIG==ANYLIST(1)) THEN
     LFOUND=.TRUE.
    ELSE
     LFOUND=.FALSE.
     ILISTSIZE=ILISTSIZE+1
     ANYLIST(ILISTSIZE)=CONFIG
    ENDIF
    RETURN
   ENDIF
   ...
   DO WHILE(.TRUE.)
    IF (CONFIG==ANYLIST(IMAX)) THEN
     LFOUND=.TRUE.
     EXIT
    ELSE IF (CONFIG > ANYLIST(IMAX)) THEN
     LFOUND=.FALSE.
     ILISTSIZE=ILISTSIZE+1
     ANYLIST(ILISTSIZE)=CONFIG
     EXIT
    ELSE IF (CONFIG==ANYLIST(IMID)) THEN
     LFOUND=.TRUE.
     EXIT
    ELSE IF (CONFIG > ANYLIST(IMID)) THEN
     IF (IMID+1==IMAX) THEN
      LFOUND=.FALSE.
      DO I=ILISTSIZE,IMAX,-1
       ANYLIST(I+1)=ANYLIST(I)
      ENDDO
      ANYLIST(IMID+1)=CONFIG
      ILISTSIZE=ILISTSIZE+1
      EXIT
     ELSE
      IMIN=IMID
      IMID=(IMIN+IMAX)/2
     ENDIF
    ELSE IF (CONFIG==ANYLIST(IMIN)) THEN
     LFOUND=.TRUE.
     EXIT
    ELSE IF (IMIN+1==IMID) THEN
     LFOUND=.FALSE. 
     DO I=ILISTSIZE,IMID,-1
      ANYLIST(I+1)=ANYLIST(I)
     ENDDO
     ANYLIST(IMIN+1)=CONFIG
     ILISTSIZE=ILISTSIZE+1
     EXIT
    ELSE
     IMAX=IMID
